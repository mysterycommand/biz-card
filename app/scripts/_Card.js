...
define([
    ...
    var Card = function Card(w, h) {
        ...
        this.svg = new SVG(this.w, this.h, 'mm');
        this.svg.setAttributeNS(null, 'id', 'business-card-' + counter++);

        this.fills = new Group(0, 0);
        this.strokes = new Group(0, 0);
    ...
    function randomAttrs(hex, maxStrokeWidth) {
        ...
        var fill = coinToss(),
            ...
            // multiplier = 72 / 25.4,
            strokeWidth = (Math.ceil(Math.random() * maxStrokeWidth) / 2), // * multiplier,
            attrs = {
                'fill': 'none',
                'fill-rule': 'evenodd',
                'stroke': 'none'
    ...
    Card.prototype.drawFront = function() {
        ...
        Object.keys(elements).forEach(function(key) {
            if (coinToss()) {
                console.log(key, elements[key].getAttributeNS(null, 'fill'), this);
                var g = elements[key].getAttributeNS(null, 'fill') === 'none' ? this.strokes : this.fills;
                if (coinToss()) { g.appendChild(elements[key]); }
            }
        }.bind(this));
    };
    ...
    Card.prototype.center = function() {
        ...
        this.svg.appendChild(this.fills);
        this.svg.appendChild(this.strokes);
    };
